...
# 在训练集上引入强度为0.1的椒盐噪声
noise = np.random.uniform(0,1,train_imgs.shape)
noise[noise < 0.9] = 0
noise[noise >= 0.9] = 1
train_imgs = train_imgs + noise
train_imgs[train_imgs > 1] = 1
train_imgs[train_imgs < 0] = 0

# linear_model = nn.models.Model_MLP([train_imgs.shape[-1], 600, 10], 'ReLU', [1e-4, 1e-4])0p
